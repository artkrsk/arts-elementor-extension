(function () {
	window.addEventListener('arts/elementor_extension/editor/setting_changed', onSettingChanged);

	const settingsMap = {
		'enabled': 'smooth_scrolling_enabled',
		'matchMedia': 'smooth_scrolling_match_media',
		'lerp': {
			'return_size': true,
			'value': 'smooth_scrolling_lerp',
		},
		'duration': {
			'return_size': true,
			'value': 'smooth_scrolling_duration',
		},
		'easing': 'smooth_scrolling_easing',
		'useGSAPRaf': 'smooth_scrolling_gsap_raf_enabled',
	};
	const liveSettings = getLiveSettings(settingsMap);

	function onSettingChanged({ detail }) {
		const { settings, setting, value } = detail;

		if (!liveSettings.includes(setting)) {
			return;
		}

		const options = convertSettings(settings);
		setEnabled(options);
		Object.assign(window.artsSmoothScrollingOptions, options);

		window.dispatchEvent(new CustomEvent('arts/smooth_scrolling/settings_changed', {
			detail: {
				options,
	...
	function processComplexValue(valueMapping, settings) {
	...
	function getLiveSettings(settingsMap = {}, additionalSettings = []) {
		const keys = [];
